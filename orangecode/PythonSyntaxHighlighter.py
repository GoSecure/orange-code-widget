...
class PythonSyntaxHighlighter(QSyntaxHighlighter):
    ...
    java_keywords = ['class','public','private','protected','null','false','true']
    ...
    def __init__(self, document, palette):
        ...
        self.tri_single = (QRegExp("'''"), 1, self._style("string2"))
        self.tri_double = (QRegExp('"""'), 2, self._style("string2"))

        rules = []

        # Keyword, operator, and brace rules
        rules += [(r'\b%s\b' % w, 0, self._style("keyword"))
            for w in self.python_keywords]
        rules += [(r'\b%s\b' % w, 0, self._style("keyword"))
            for w in self.java_keywords]
